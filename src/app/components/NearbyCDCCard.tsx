          ...
          <div className="space-y-3">
            {visible.map((cdc, i) => (
              ...
              >
                ...
                {cdc.description && (
                  <p className="text-xs text-slate-300 mb-1.5">{cdc.description}</p>
                )}
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400">
                  <span className="flex items-center gap-1"><MapPin size={11} /> {cdc.distance}m away</span>
                  {cdc.applicationType && <span className="text-slate-500">{cdc.applicationType}</span>}
                ...
                <div className="flex flex-wrap gap-x-3 text-[10px] text-slate-500 mt-1.5">
                  {cdc.councilRef && <span>Ref: {cdc.councilRef}</span>}
                  {cdc.pan && <span>PAN: {cdc.pan}</span>}
                  {cdc.lot && <span>{cdc.lot}</span>}
                </div>
              </motion.div>
            ))}
          </div>
