  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex items-center gap-3 mb-1">
        <ClipboardList style={{ color: "var(--text-muted)" }} size={22} />
          ...
          <div className="space-y-3">
            {visible.map((da, i) => (
              ...
              >
                ...
                {da.description && (
                  <p className="text-xs text-slate-300 mb-1.5">{da.description}</p>
                )}
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400">
                  <span className="flex items-center gap-1"><MapPin size={11} /> {da.distance}m away</span>
                  {da.applicationType && <span className="text-slate-500">{da.applicationType}</span>}
                ...
                <div className="flex flex-wrap gap-x-3 text-[10px] text-slate-500 mt-1.5">
                  {da.councilRef && <span>Ref: {da.councilRef}</span>}
                  {da.pan && <span>PAN: {da.pan}</span>}
                  {da.lot && <span>{da.lot}</span>}
                </div>
              </motion.div>
            ))}
          </div>
