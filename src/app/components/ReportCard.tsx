...
const ZONE_DESC: Record<string, string> = {
  ...
  E1: "Conservation, limited building",
  E2: "Conservation, limited building",
  ...
  RU1: "Farming, rural housing",
  RU2: "Farming, rural housing",
};

function getZoneDesc(code: string) {
  return ZONE_DESC[code?.replace(/\s.*/, "") || ""] || "Check local LEP for permitted uses";
}

export default function ReportCard({ data }: { data: any }) {
  const { address, planning, hazard, cadastre, councilName } = data;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full max-w-6xl mx-auto">
      <ZoneCard icon={<MapPin size={20} />} title="Address & Location" delay={0}>
        ...
        {(councilName || zoning.LGA_NAME) && <p>LGA: {councilName || zoning.LGA_NAME}</p>}
      ...
      <ZoneCard icon={<Ruler size={20} />} title="Height Limit" delay={0.1}>
        ...
        ) : <p className="text-slate-500">No height limit mapped</p>}
      </ZoneCard>
      ...
      <ZoneCard icon={<BarChart3 size={20} />} title="Floor Space Ratio" delay={0.15}>
        ...
        ) : <p className="text-slate-500">No FSR mapped</p>}
      </ZoneCard>
      ...
      <ZoneCard icon={<Move size={20} />} title="Minimum Lot Size" delay={0.2}>
        ...
        ) : <p className="text-slate-500">No minimum lot size mapped</p>}
      </ZoneCard>
      ...
      <ZoneCard icon={<FlaskConical size={20} />} title="Acid Sulfate Soils" delay={0.45}>
        {acidSulfate.length > 0 ? (
          <>
            ...
            <p>{acidSulfate[0].attributes?.LABEL || "Present"}</p>
          </>
