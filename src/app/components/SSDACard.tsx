...
export default function SSDACard({
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-semibold text-white text-lg">State Significant Developments</h3>
        </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
      ...
      <p className="text-xs text-slate-500 mb-4">Major projects in {lga}</p>
      ...
      {projects.length === 0 ? (
        ...
        <div className="space-y-3">
          {visible.map((p) => (
            <div key={p.caseId || p.title} className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.05]">
              <div className="flex flex-wrap items-start gap-2 mb-1.5">
                ...
                {p.assessmentType && <span className="text-xs text-slate-500">{p.assessmentType}</span>}
              </div>
              ...
              {p.detailUrl && (
                <a href={`https://www.planningportal.nsw.gov.au${p.detailUrl}`} target="_blank" rel="noopener noreferrer"
      ...
      {projects.length > 0 && (
        <a href="https://www.planningportal.nsw.gov.au/major-projects/projects" target="_blank" rel="noopener noreferrer"
